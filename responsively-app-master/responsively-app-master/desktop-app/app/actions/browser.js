  ...
  return (dispatch: Dispatch, getState: RootStateType) => {
    ...
    if (newMode !== DEVTOOLS_MODES.UNDOCKED) {
      ...
      newConfig = {
		...
		//experimental
		//const old_window_size = getState();
		//dispatch(newWindowSize(windowSize));
		//newWindowSize({windowSize.width, windowSize.height});
		
		//testing for if this code is active when righ docking
		if(newMode == RIGHT_DOCKED) {
			console.log("right docked");
		}
		else {
			console.log("not right docked");
      };
    }
